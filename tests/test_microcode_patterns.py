...
def test_immediate8_steps():
    ...
    assert "PC_TO_MAR" in p[0]["signals"]
    assert "MEM_READ" in p[1]["signals"]

def test_zero_page_is_explicit():
    p = zero_page_address()
    assert [x["step"] for x in p] == [3, 4, 5]
    assert p[-1]["signals"] == ["MDR_OUT", "MAR_LOAD_LO"]

def test_absolute_keeps_both_address_bytes_visible():
    p = absolute_address()
    assert len(p) == 7
    assert "TMP_LOAD" in p[2]["signals"]
    assert p[-2]["signals"] == ["TMP_OUT", "MAR_LOAD_LO"]
    assert p[-1]["signals"] == ["MDR_OUT", "MAR_LOAD_HI"]
